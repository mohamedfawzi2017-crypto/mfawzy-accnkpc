...
framework with lagged policy rates ($i_{t-1}$) to eliminate contemporaneous endogeneity across
...
on macroeconomic calibration, implied Calvo stickiness is $\theta^+ \approx 0.62$ ($2.6$ months)
for upward adjustments versus $\theta^- \approx 0.88$ ($8.3$ months) for downward adjustments,
...
asymmetric ($\lambda^{+} = 8.2912,\; p = 0.002$). Results are robust across extensive IV
exclusion checks (omitting oil prices, IMF dummies, and spread proxies; Kleibergen--Paap $F = 28.4$,
...
followed in the short run by an acceleration of headline inflation. This phenomenon---conventionally
termed the ``Price Puzzle''---has historically been attributed to omitted forward-looking
variables in VAR identification \citep{sims1992, bernanke1998}. A structural explanation
is grounded in the \textit{Cost Channel of Monetary Policy} \citep{barth2001, ravenna2006, chowdhury2006}:
...
with lagged policy rates ($i_{t-1}$) to prevent contemporaneous endogeneity.
\item \textbf{Robust Econometric Identification:} We estimate the structural equation
via Two-Step Optimal GMM and HAC-OLS, utilizing validated external instruments
...
\label{eq:mc_linear}
\end{equation}
where $\tilde{\mu} \equiv \frac{\chi}{1 + \chi i_{ss}}$ is the structural working-capital
intensity and the disturbance $u_t \equiv -\hat{a}_t + \tilde{\mu}\widehat{\text{Spread}}_t$
absorbs TFP shocks and banking spread innovations.
...
\subsection{Deep Parameter Inversion & Rockets and Feathers}

Table~\ref{tab:calibration} summarizes the macroeconomic calibration and structural
parameter inversion. Setting $\beta = 0.995, \sigma = 1.67, \varphi = 2.0$, import share
$\alpha = 0.35$, steady-state policy rate $i_{ss} = 12.0\%$, and working-capital loan share
$\chi = 0.65$ ($\tilde{\mu} = 0.603$), inverting \eqref{eq:slopes} from our estimated
reduced-form slopes yields:
\begin{itemize}
    \item \textbf{Upward Price Stickiness:} $\theta^{+} \approx 0.620 \implies$ implied average
    price duration $d^{+} = \frac{1}{1-\theta^{+}} \approx 2.63\text{ months}$.
    \item \textbf{Downward Price Stickiness:} $\theta^{-} \approx 0.880 \implies$ implied average
    price duration $d^{-} = \frac{1}{1-\theta^{-}} \approx 8.33\text{ months}$.
\end{itemize}
This provides structural evidence consistent with a ``Rockets and Feathers'' pricing
pattern: domestic enterprises adjust prices upward in response to cost increases in under 3 months,
while requiring more than 8 months to pass through cost reductions.
...
\midrule
\textbf{Implied Calvo Parameter ($\theta^+$)} & $\theta^+$ & $\mathbf{0.620}$ & Inverted from $\phi_{\text{Low}}^+ = 1.8783$ ($d^+ = 2.63\text{ months}$) \\
\textbf{Implied Calvo Parameter ($\theta^-$)} & $\theta^-$ & $\mathbf{0.880}$ & Inverted from $\phi^- = 0.1733$ ($d^- = 8.33\text{ months}$) \\
\bottomrule
\end{tabularx}
\begin{tablenotes}
\footnotesize
\item \textit{Note:} Calvo parameters $\theta^+$ and $\theta^-$ represent implied values conditional on the structural calibration baseline.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
When $\phi^+ > \kappa/\sigma$, the marginal cost push from working-capital financing
exceeds the disinflationary pressure from aggregate demand contraction.
\end{proof}
...
\end{equation}
In the low-rate regime ($i_{t-1} \le 20.0\%$), the elevated cost-channel slope ($\phi_{\text{Low}}^+ = 1.8783$) narrows the determinacy region, requiring vigilant coordination. Conversely, in the high-rate regime ($i_{t-1} > 20.0\%$), the sharp contraction of the cost channel ($\phi_{\text{High}}^+ \to 0.1232$) ensures that the classical Taylor condition $\phi_\pi > 1$ readily guarantees unique, stable rational-expectations equilibrium dynamics.
...
\textbf{BDS Test Interpretation.} Panel B reports the \citet{brock1996} BDS test applied
to the standardized residuals of an $\text{AR}(2)$ inflation filter across embedding dimensions
$m \in \{2, 3, 4\}$. The BDS $z$-statistics range from $22.4$ to $25.8$, rejecting the null
hypothesis of independent and identically distributed (i.i.d.) errors at $p < 0.0001$.
This departure from i.i.d.\ structure provides statistical motivation for exploring
non-linear threshold and state-dependent pricing dynamics. Panel C confirms that Variance
Inflation Factors (VIF) remain strictly below $1.55$, ruling out collinearity among partial sums.
...
To avoid contemporaneous endogeneity bias, the optimal policy rate threshold $i^*$ is
estimated using the \textbf{lagged policy rate ($i_{t-1}$)} as the transition variable
following \citet{hansen2000}:
\begin{equation}
\pi_t = X_t'\beta_1 \cdot \mathbb{I}(i_{t-1} \le i^*) + X_t'\beta_2 \cdot \mathbb{I}(i_{t-1} > i^*) + \varepsilon_t
\label{eq:hansen_model}
\end{equation}
The threshold search interval $i_{t-1}^* \in [10\%, 24\%]$ corresponds to the interior
$10\text{th}$ to $90\text{th}$ empirical quantiles of the policy rate distribution, ensuring
that each regime contains sufficient observations ($N \ge 40$) to prevent sparse-sample distortions.
...
\node[font=\footnotesize, fill=white, draw=red!70, rounded corners=2pt]
    at (axis cs:20,146){\textbf{Optimal $i^*=20.0\%$}};
...
The global minimum is $i^* = 20.0\%$, with 95\% bootstrap LR confidence interval $[18.5\%, 21.5\%]$.}
\label{fig:threshold}
\end{figure}
...
To estimate the Hybrid ACC-NKPC \eqref{eq:acc_nkpc} with forward-looking expectations
$\mathbb{E}_t[\pi_{t+1}]$ and lagged inflation $\pi_{t-1}$, we employ \textbf{Two-Step Optimal GMM}
with a Newey--West HAC weighting matrix. The instrument set is specified as
$Z_t = \{\pi_{t-2}$, $\pi_{t-3}$, $\tilde{y}_{t-1}$, $\Delta\text{FFR}_{t-1}$, $\Delta\ln\text{OIL}_{t-1}$,
$\text{Spread}_{t-1}$, and $\text{IMF Dummies}\}$. The Kleibergen--Paap $F$-statistic is $28.4$,
and the Hansen $J$-test fails to reject the null of valid overidentifying restrictions ($J = 4.25, p = 0.512$).
...
\item \textbf{Hybrid Dynamics:} Forward-looking expectations ($\gamma_f = 0.5210, p < 0.001$)
and backward persistence ($\gamma_b = 0.4420, p < 0.001$) sum to $0.9630 \approx 1$, confirming
theoretical validity without unit-root instability.
\end{enumerate}
...
\midrule
\multicolumn{5}{l}{\textit{Panel C: Formal Structural Wald Hypothesis Tests}} \\[2pt]
...
\begin{tablenotes}
\footnotesize
\item Standard errors are computed using Newey--West HAC covariance matrices. Threshold specification in Panel B controls for forward expectations $\mathbb{E}_t[\pi_{t+1}]$ and backward persistence $\pi_{t-1}$ as estimated in Panel A. Instruments: $Z_t = \{\pi_{t-2}$, $\pi_{t-3}$, $\tilde{y}_{t-1}$, $\Delta\text{FFR}_{t-1}$, $\Delta\ln\text{OIL}_{t-1}$, $\text{Spread}_{t-1}$, $\text{IMF Dummies}\}$. Kleibergen--Paap $F = 28.4$.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
Table~\ref{tab:post_diag} presents the post-estimation diagnostic suite. The Ramsey RESET
test ($F = 0.141, p = 0.7074$) provides no evidence of additional neglected non-linear terms.
...
Shaded regions indicate 90\% confidence intervals for currency depreciation and rate hikes.}
\label{fig:dynamic_multipliers}
\end{figure}

% ══════════════════════════════════════════════════════════════
\section{Machine Learning Predictive Validation and Non-Linear Analysis}
\label{sec:ml}
% ══════════════════════════════════════════════════════════════
...
To evaluate predictive performance and investigate whether machine learning algorithms
recover a similar non-linear threshold without prior parametric restrictions, we benchmark
tree-based ensembles (Gradient Boosting, Random Forest), kernel methods (SVR), neural
architectures (LSTM, MLP), and standard econometric models.
...
structural drivers. The partial dependence function for the policy rate exhibits a positive
marginal slope below $19.8\%$, which inverts to a negative slope above $20.0\%$. This demonstrates
that flexible machine learning algorithms recover a qualitatively similar non-linear dependence
around the econometrically estimated threshold.

\textbf{Two-Dimensional SHAP Feature Interactions.} Furthermore, two-dimensional SHAP interaction values reveal a strong synergistic coupling between policy rate hikes ($\Delta i_t^+$) and exchange rate depreciation ($\Delta e_t^+$): during sharp currency floatation episodes, the marginal cost-push effect of monetary tightening in the tree ensemble is amplified by approximately $35\%$, reflecting severe simultaneous working-capital and imported-input cost pressures on corporate pricing schedules.
...
\textbf{Lesson 2: Optimal Normative Easing vs.\ Hawkish Central Bank Bias.}
The simulated easing path of $150$--$200$ bps per quarter in Figure~\ref{fig:multi_panel_forecast} represents an \textit{optimal normative guideline} rather than a deterministic forecast of Monetary Policy Committee (MPC) voting behavior. Historically, the CBE maintains a cautious, hawkish stance to ensure positive real interest rates ($+200$ to $+400$ bps) to anchor domestic deposits and discourage dollarization. In practice, the easing cycle may proceed via data-dependent, phased pauses as long as external conditions remain volatile.
...
\textit{Updated Welfare Cost Calibration:} Structurally, asymmetric price-setting imposes a welfare loss equivalent to $0.84\%$ of annual household consumption expenditure ($\text{CV}/\bar{C}_{hh} = 0.84\%$). While this evaluated to $\text{EGP } 504$ at baseline 2020 HIECS prices ($\bar{C}_{hh} = \text{EGP } 60{,}000$), cumulative nominal inflation updates current annual household expenditure to $\bar{C}_{hh, 2026} \approx \text{EGP } 160{,}000$--$180{,}000$, resulting in an updated nominal welfare burden of:
\begin{equation}
...
The Threshold Inversion Proposition establishes that interest rate hikes increase inflation
when $i_{t-1} \le 20.0\%$ ($\phi_{\text{Low}}^{+} = 1.8783, p < 0.001$), while aggregate demand
contraction re-establishes disinflationary transmission above $20.0\%$. Exchange rate pass-through
is strongly asymmetric ($\lambda^{+} = 8.2912, p = 0.002$). Machine learning models achieve
superior forecasting performance ($\text{RMSE}=3.060\text{ pp}$) and recover a qualitatively similar
non-linear dependence around $20.0\%$. These findings provide actionable roadmaps for central bank monetary
policy sequencing and macroprudential credit management.

\section*{Data Availability Statement}
All monthly datasets, replication scripts, and empirical routines are available
at the author's open research repository: \href{https://github.com/mfawzy-accnkpc}{\texttt{https://github.com/mfawzy-accnkpc}}.
...
\end{equation}
Dividing through by $(1 + \beta\gamma)$ and aggregating over optimal reset prices $p_t^{*+}$ (for positive cost shock mass $\omega_t$) and $p_t^{*-}$ (for mass $1-\omega_t$) yields the structural Hybrid ACC-NKPC \eqref{eq:acc_nkpc} with structural weights:
\begin{equation}
